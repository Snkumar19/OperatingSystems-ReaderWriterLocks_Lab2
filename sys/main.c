...
void test5() {
	...
	for (index = 0; index < 5; index++) {
		lck[index] = lcreate();
		assert(lck[index] != SYSERR, "Test 5 FAILED- 1\n");

		ret = lock(lck[index], READ, DEFAULT_LOCK_PRIO);
		assert(ret == OK, "Test 5 FAILED - 2\n");
	}

	ret = releaseall(2, lck[4], lck[0]);
	kprintf("\n RET = %d", ret);
	assert(ret == OK, "Test 5 FAILED - 3\n");

	ret = releaseall(3, lck[1], lck[3], lck[2]);
	assert(ret == OK, "Test 5 FAILED - 4\n");
...
void procAA() {
	...
	if (releaseall(2, lck1, lck3) == SYSERR)
		kprintf("procA doesn't hold this lock\n");
}
